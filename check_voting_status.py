...
def list_reports(bucket_name, prefix='reports/', hours=24):
...
def main():
    parser = argparse.ArgumentParser(description='Check voting status')
    ...
    executions = list_reports(args.bucket, hours=args.hours)
    
    if not executions:
        print(f"No executions found in the last {args.hours} hours")
        return
    
    print(f"Found {len(executions)} executions in the last {args.hours} hours")
    print()
